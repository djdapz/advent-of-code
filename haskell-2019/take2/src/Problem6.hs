...
orbitalPairToPlanets :: String -> (String, String)
orbitalPairToPlanets pair =
  let planetArray = splitOn ')' pair
   in (last planetArray, head planetArray)

newOrbitalMap :: [String] -> OrbitalMap
newOrbitalMap input = fromList [orbitalPairToPlanets orbitalPair | orbitalPair <- input]

orbitsWhat :: String -> OrbitalMap -> String
orbitsWhat planet oMap = oMap ! planet

numberOfOrbits :: String -> OrbitalMap -> Int
numberOfOrbits planet oMap
  | target == "COM" = 1
  | otherwise = 1 + numberOfOrbits target oMap
  where
