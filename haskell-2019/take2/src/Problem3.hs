module Problem3
  ...
  , Segment(..)
  , readWire
  , wireDefinitionToPoints
  , intersectionPoints
  , unboundedIntersectionPoint
  , pointIsOnLine
  , intersects
  , shortestManhattan
  , pointsToManhattan
...
zipIntersections :: [Point] -> Wire -> Wire -> [Int]
zipIntersections intersections wire1 wire2 =
  [numberOfSteps intersection wire1 + numberOfSteps intersection wire2 | intersection <- intersections]

numberOfSteps :: Point -> Wire -> Int
numberOfSteps point segments
  | pointIsOnLine point segment = distanceTillPoint point segment
  | otherwise = segmentLength segment + numberOfSteps point (tail segments)
  where
    segment = head segments

distanceTillPoint :: Point -> Segment -> Int
distanceTillPoint point segment
  | isVertical segment = abs (snd point - snd (fst segment))
  | isHorizontal segment = abs (fst point - fst (fst segment))
...
closestIntersection wireString1 wireString2 =
  let wire1 = readWire wireString1
      wire2 = readWire wireString2
   in minimum (zipIntersections (intersectionPoints wire1 wire2) wire1 wire2)

intersectionPoints :: Wire -> Wire -> [Point]
intersectionPoints wire1 wire2 =
  [unboundedIntersectionPoint segment1 segment2 | segment1 <- wire1, segment2 <- wire2, intersects segment1 segment2]

shortestManhattan :: [(Int, Int)] -> Int
shortestManhattan points = minimum (filter (> 0) (pointsToManhattan points))

pointsToManhattan :: [(Int, Int)] -> [Int]
pointsToManhattan = map manhattan

manhattan :: (Int, Int) -> Int
manhattan (x, y) = abs x + abs y

pointIsOnLine :: Point -> Segment -> Bool
--            ( 4,  4) (( 0,  0), ( 0,  4))
pointIsOnLine (px, py) ((x0, y0), (x1, y1))
  | isVertical ((x0, y0), (x1, y1)) && y1 > y0 = py >= y0 && py <= y1 && px == x0
  | isVertical ((x0, y0), (x1, y1)) && y1 < y0 = py <= y0 && py >= y1 && px == x0
  | isHorizontal ((x0, y0), (x1, y1)) && x1 > x0 = px >= x0 && px <= x1 && py == y0
  | isHorizontal ((x0, y0), (x1, y1)) && x1 < x0 = px <= x0 && px >= x1 && py == y0

unboundedIntersectionPoint :: Segment -> Segment -> Point
unboundedIntersectionPoint segment1 segment2
  | isVertical segment1 && isHorizontal segment2 = (fst (fst segment1), snd (snd segment2))
  | isVertical segment2 && isHorizontal segment1 = (fst (fst segment2), snd (snd segment1))

isVertical :: Segment -> Bool
isVertical seg = fst (fst seg) == fst (snd seg)

isHorizontal :: Segment -> Bool
isHorizontal seg = snd (fst seg) == snd (snd seg)

intersects :: Segment -> Segment -> Bool
intersects segment1 segment2
  | isHorizontal segment1 && isHorizontal segment2 = False
  | isVertical segment1 && isVertical segment2 = False
  | otherwise = pointIsOnLine unboundedIP segment1 && pointIsOnLine unboundedIP segment2
  where
    unboundedIP = unboundedIntersectionPoint segment1 segment2

readWire :: [String] -> Wire
readWire = wireDefinitionToPoints (0, 0)

wireDefinitionToPoints :: Point -> [String] -> Wire
wireDefinitionToPoints startingPoint directions
  | null directions = []
  | otherwise = thisSection : wireDefinitionToPoints (snd thisSection) (tail directions)
  where
    thisSection = commandToSegments (head directions) startingPoint

commandToSegments :: String -> Point -> Segment
commandToSegments ('R':distance) start = (start, (fst start + read distance, snd start))
commandToSegments ('L':distance) start = (start, (fst start - read distance, snd start))
commandToSegments ('U':distance) start = (start, (fst start, snd start + read distance))
commandToSegments ('D':distance) start = (start, (fst start, snd start - read distance))

splitOnCommas :: String -> [String]
splitOnCommas = splitWhere (== ',')
